...
const cancelTianyiTask = callable("cancel_tianyi_task");
const clearTianyiInactiveTasks = callable("clear_tianyi_inactive_tasks");
...
function TaskProgressRow(task, onRemove) {
    ...
    return (SP_JSX.jsxs("div", { style: {
            ...
            gap: "6px",
                    ...
                    } }) }), SP_JSX.jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px", fontSize: "11px", color: "#c9c9c9" }, children: [SP_JSX.jsx("span", { children: `进度 ${progress.toFixed(1)}%` }), SP_JSX.jsx("span", { children: `速度 ${formatSpeed(task.speed)}` })] }), installStatus ? (SP_JSX.jsx("div", { style: { fontSize: "11px", color: "#bcbcbc", lineHeight: 1.4 }, children: `安装：${installStatusText(installStatus)}${installMessage ? ` | ${installMessage}` : ""}` })) : null, errorReason ? (SP_JSX.jsx("div", { style: { fontSize: "11px", color: "#ff8e8e", lineHeight: 1.4 }, children: `错误：${errorReason}` })) : null, onRemove && (SP_JSX.jsx("div", { style: { display: "flex", justifyContent: "flex-end", marginTop: "4px" }, children: SP_JSX.jsx(DFL.Focusable, { children: SP_JSX.jsx(DFL.DialogButton, { onClick: () => onRemove(task.task_id), style: {
                            minWidth: "60px",
                            padding: "2px 12px",
                            fontSize: "12px",
                            borderRadius: "6px",
...
function Content() {
    ...
    const handleRemoveTask = SP_REACT.useCallback(async (taskId) => {
        DFL.showModal(SP_JSX.jsx(DFL.ConfirmModal, { strTitle: "\u79FB\u9664\u4EFB\u52A1", strDescription: "\u786E\u5B9A\u8981\u79FB\u9664\u8FD9\u4E2A\u4E0B\u8F7D\u4EFB\u52A1\u5417\uFF1F\u5982\u679C\u6B63\u5728\u4E0B\u8F7D\uFF0C\u5C06\u4F1A\u505C\u6B62\u5E76\u5220\u9664\u5DF2\u4E0B\u8F7D\u7684\u6587\u4EF6\uFF1B\u5982\u679C\u662F\u5931\u8D25\u4EFB\u52A1\uFF0C\u5C06\u6E05\u9664\u5176\u9519\u8BEF\u8BB0\u5F55\u53CA\u53EF\u80FD\u5B58\u5728\u7684\u672A\u5B8C\u6210\u6587\u4EF6\u3002", strOKButtonText: "\u79FB\u9664", strCancelButtonText: "\u53D6\u6D88", onOK: async () => {
                try {
                    const result = await cancelTianyiTask({ task_id: taskId, delete_files: true });
                    if (result.status !== "success") {
                        throw new Error(result.message || "移除失败");
                    }
                    toaster.toast({ title: "Freedeck", body: "任务已移除" });
                    await syncState();
                }
                catch (error) {
                    toaster.toast({ title: "Freedeck", body: String(error) });
