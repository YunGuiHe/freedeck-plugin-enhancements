...
function Content() {
    ...
    const onClearInactiveTasks = SP_REACT.useCallback(async () => {
        if (uninstallingKey)
            return;
        try {
            ...
            const files = data?.cleared_files || 0;
            toaster.toast({ title: "Freedeck", body: `清理缓存成功，共 ${count} 个任务，${files} 个文件` });
            await syncState();
        }
        catch (error) {
            toaster.toast({ title: "Freedeck", body: `清理出错：${error}` });
        }
    }, [syncState, uninstallingKey]);
    if (loading) {
