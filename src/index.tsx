...
const cancelTianyiTask = callable<[{ task_id: string; delete_files: boolean }], ApiResponse<Record<string, unknown>>>(
  "cancel_tianyi_task",
);
const clearTianyiInactiveTasks = callable<[], ApiResponse<Record<string, unknown>>>(
  "clear_tianyi_inactive_tasks",
);
...
function TaskProgressRow(task: TaskItem, onRemove?: (taskId: string) => void) {
  ...
  const handleRemoveTask = useCallback(
    async (taskId: string) => {
      showModal(
        <ConfirmModal
          strTitle="移除任务"
          strDescription="确定要移除这个下载任务吗？如果正在下载，将会停止并删除已下载的文件；如果是失败任务，将清除其错误记录及可能存在的未完成文件。"
          strOKButtonText="移除"
          strCancelButtonText="取消"
          onOK={async () => {
            try {
        ...
        <PanelSectionRow>
          ...
          <ButtonItem
            ...
            onClick={() => {
              showModal(
                <ConfirmModal
                  strTitle="清理冗余记录及缓存文件"
                  strDescription="确定要一键销毁所有【已完成】（未安装）、【已移除】以及【错误】的下载记录，并清空所有残留碎片和未完成的文件吗？(不会影响正在下载或排队中的任务)"
                  strOKButtonText="一键销毁"
                  strCancelButtonText="取消"
                  onOK={async () => {
                    try {
        ...
        <PanelSection title={`下载列表（${visibleTasks.length}）`}>
          {visibleTasks.slice(0, 20).map((task) => (
            <PanelSectionRow key={task.task_id}>
              {TaskProgressRow(task, handleRemoveTask)}
            </PanelSectionRow>
          ))}
        </PanelSection>
      )}
              ...
              <PanelSectionRow key={`${item.game_id || item.title || "game"}_${index}`}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "10px", width: "100%" }}>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    ...
                    <div style={{ fontSize: "12px" }}>
                      {`游玩时长：${formatPlaytimeText(item.playtime_seconds || 0, item.playtime_text)}${item.playtime_active ? "（进行中）" : ""
                        }`}
                    </div>
                    <div
                      style={{
