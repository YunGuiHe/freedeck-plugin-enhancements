...
class TianyiService:
    ...
    async def clear_inactive_tasks(self) -> Dict[str, Any]:
        """一键清理所有已完结/失败的任务记录及其缓存文件。"""
        import shutil, os, config
        cleared_count = 0
        cleared_files = 0
        new_tasks = []
        ...
        for task in self.store.tasks:
            ...
            install_status = str(task.install_status or "").lower()

            if status in ("error", "removed"):
                is_inactive = True
            elif status == "complete" and install_status != "installed":
                # 如果是下载完成了但没有安装成功，也当作无效缓存处理
                is_inactive = True
            ...
            if is_inactive:
                ...
                if task.local_path and os.path.exists(task.local_path):
                    try:
                        ...
                        cleared_files += 1
                        # 尝试清理空目录
                        parent_dir = os.path.dirname(task.local_path)
        ...
        if cleared_count > 0:
            self.store.replace_tasks(new_tasks)
            self._invalidate_panel_cache(tasks=True)
