...
class TianyiService:
    ...
    async def cancel_task(self, task_id: str, delete_files: bool = False) -> Dict[str, Any]:
        """取消甚至移除任务，可选删除已下载的文件。"""
        task = self._find_task(task_id)
        if task is None:
            raise ValueError("任务不存在")
        ...
        # 扩展原版逻辑，当请求清空文件或任务状态为错误时触发删除，以此清除记录和文件
        if delete_files and task.local_path and os.path.exists(task.local_path):
            try:
                if os.path.isdir(task.local_path):
                    import shutil
                    shutil.rmtree(task.local_path, ignore_errors=True)
                else:
                    os.remove(task.local_path)
            except Exception as e:
                config.logger.warning(f"删除任务文件失败: {e}")

        task.status = "removed"
        task.updated_at = _now_wall_ts()
        
        # 增强修复：如果是人为要求移除直接从当前的数据库列表中永久摘除，不保留 7 天占位
        tasks = [t for t in self.store.tasks if t.task_id != task_id]
        self.store.replace_tasks(tasks)
    ...
    async def clear_inactive_tasks(self) -> Dict[str, Any]:
        """增强修复：一键清理所有失败、暂停过久或已完成的未提取任务，并删除关联废弃缓存"""
        tasks = list(self.store.tasks)
        cleared_count = 0
        active_tasks = []
        for task in tasks:
            ...
            if task.local_path and os.path.exists(task.local_path):
                try:
                    ...
                    config.logger.warning(f"一键清理缓存: 删除文件失败: {e}")
            cleared_count += 1

        if cleared_count > 0:
            self.store.replace_tasks(active_tasks)
            self._cleanup_tasks(self.store.tasks)
            self.store.save()
            self._invalidate_panel_cache(tasks=True)
